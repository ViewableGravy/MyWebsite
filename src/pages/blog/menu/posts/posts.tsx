...
import { TDraft, TPost } from "./types";
...
export const Posts = () => {
  ...
  const onDraft = async () => { 
    setPosts(await API.Posts.getDrafts()); 
    setTitle('Drafts');
  }
  const onPublished = async () => {
    setPosts(await API.Posts.getPosts());
    setTitle('Posts'); 
  }

  React.useEffect(() => {
    if (posts === null) {
      draftMode ? onDraft() : onPublished();
    }
  }, [draftMode, posts])
